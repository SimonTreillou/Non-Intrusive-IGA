...
### On cherche les coordonnées des noeuds de l'interface
XYZnodes = {node:mesh.GetNodeXYZ(node) for node in nodeInterf}
...
#    XYZ = mesh.GetNodeXYZ(node)
    Vertex[node] = geompy.MakeVertex(XYZnodes[node][0],XYZnodes[node][1],XYZnodes[node][2])
...
newMesh = smesh.Mesh()
smesh.SetName(newMesh.GetMesh(), 'newMesh')
nodeID = dict()
for ii,node in enumerate(nodeInterf):
    nodeID[node] = newMesh.AddNode(XYZnodes[node][0],XYZnodes[node][1],XYZnodes[node][2])
edgeID = dict()
for ii,elem in enumerate(elemInterf):
    edgeID[elem] = newMesh.AddEdge([nodeID[connecInterf[elem][0]],nodeID[connecInterf[elem][1]],nodeID[connecInterf[elem][2]]])
