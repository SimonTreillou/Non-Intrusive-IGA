...
def MultiscaleInit(m,imf,img,cam,nscale,l0=None,U=None):
                ...
                if ig>1 and ik==0: # not first time step and first GN iteration
                    if js==0: # Highest grid level
                        ...
                        for ik in range(0,30):
                            ...
                            if err<1e-3:
                                break
                        print("Iter # %2d | disc/dyn=%2.2f %% | dU/U=%1.2e" % (ik+1,np.std(res3)/dic3.dyn*100,err))
                        UU[:,ig]=UU[:,ig-1]+U
                        [b,res]=dic2.ComputeRHS(g,m2,cam2,UU[:,ig])
                    else:
                        [b,res]=dic2.ComputeRHS(g,m2,cam2,UU[:,ig])
...
def MultiscaleInitTimePGD(m,imf,imfile,imnums,cam,nscale,U,l):   
    """ beta version: not robust """
    L=m.Tikhonov()
    ll=l.dot(l)
    for js in range(0,nscale):  # multiscale loop
        iscale=nscale-js
        f=imf.Copy()
        f.SubSample(iscale)
        cam2=cam.SubSampleCopy(iscale)
        ...
        H=dic2.ComputeLHS(f,m2,cam2)
        lam=10*10**iscale
        #M_LU=splalg.splu(dic2.M)
        H_LU=splalg.splu(H+lam*L)
        for ik in range(0,30):          # Space Gauss Netwon loop
            B=np.zeros(m.ndof)
            res=0
            for ig in range(1,len(imnums)):    # time loop
                #print("### IMAGE %2d ###" % (ig+1))
                ...
                g=Image(imdef).Load()
                g.SubSample(iscale)
                [b,r]=dic2.ComputeRHS(g,m2,cam2,U*l[ig])
                B+=l[ig]*(b-lam*L.dot(U*l[ig]))
                res=res+r
            res=res/len(imnums)
            dU=H_LU.solve(B/ll)
            U+=dU
